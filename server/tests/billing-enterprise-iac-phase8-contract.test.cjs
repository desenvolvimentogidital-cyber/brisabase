...
assert(billing.includes("redirect:'error'")&&billing.includes('AbortSignal.timeout(20_000)'),'Stripe calls must reject redirects and have timeouts');
...
assert(billing.includes("'Idempotency-Key':idempotencyKey"),'Stripe writes must support idempotency keys');
...
assert(validator.includes("BILLING_PROVIDER must be disabled or stripe"),'billing provider allowlist missing');
assert(validator.includes('STRIPE_WEBHOOK_SECRET')&&validator.includes("startsWith('whsec_')"),'Stripe signing secret validation missing');
assert(validator.includes('STRIPE_PRICE_PRO')&&validator.includes('STRIPE_PRICE_TEAM'),'paid plan price configuration validation missing');
