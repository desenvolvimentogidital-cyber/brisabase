...
const fail=(res:any,error:any)=>res.status(/not found/i.test(error?.message||'')?404:/not configured|disabled/i.test(error?.message||'')?503:/invalid|required|available|origin/i.test(error?.message||'')?400:500).json({error:{code:'BILLING_ERROR',message:process.env.NODE_ENV==='production'&&!/not configured|not found|invalid|required|available/i.test(error?.message||'')?'Billing operation failed.':error?.message||'Billing operation failed.'}});

billingWebhookRouter.post('/billing/v1/stripe/webhook',express.raw({type:'application/json',limit:'2mb'}),async(req:any,res)=>{try{const raw=Buffer.isBuffer(req.body)?req.body:Buffer.from(req.body||'');const event=await localBillingEngine.verifyStripeWebhook(raw,String(req.headers['stripe-signature']||''));await localBillingEngine.applyStripeEvent(event);return res.json({received:true});}catch(error){return res.status(400).json({error:{code:'INVALID_BILLING_WEBHOOK',message:'Invalid billing webhook.'}});}});
